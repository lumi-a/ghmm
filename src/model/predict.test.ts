...
describe('z1r belief states', () => {
  it('[0] → [1/3, 2/3, 0]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [0]), [1 / 3, 2 / 3, 0])).toBe(true);
  });
  it('[1] → [1/3, 0, 2/3]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [1]), [1 / 3, 0, 2 / 3])).toBe(true);
  });
  it('[0,0] → [0, 1, 0]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [0, 0]), [0, 1, 0])).toBe(true);
  });
  it('[0,1] → [0, 0, 1]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [0, 1]), [0, 0, 1])).toBe(true);
  });
  it('[1,0] → [1/2, 1/2, 0]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [1, 0]), [1 / 2, 1 / 2, 0])).toBe(true);
  });
  it('[1,1] → [1, 0, 0]', () => {
    expect(close(beliefState(z1r_T, z1r_initial, z1r_phi, [1, 1]), [1, 0, 0])).toBe(true);
  });
});
  ...
  for (const a of [0, 1]) {
    for (const b of [0, 1]) {
      ...
      it(`P([${a},${b},${expected}]) = ${aProb}*${bProb}`, () => {
        const got = calculateProbability(xor_T, xor_initial, [a, b, expected], xor_phi);
        expect(Math.abs(got - aProb * bProb)).toBeLessThan(1e-10);
      });

      it(`P([${a},${b},${1 - expected}]) = 0`, () => {
        const got = calculateProbability(xor_T, xor_initial, [a, b, 1 - expected], xor_phi);
