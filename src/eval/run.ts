...
export function validateSpec(spec: ModelSpec): string | null {
  ...
  if (nObs > 4)
    return `Observation count ${nObs} > 4 not supported; reduce in code or project yourself.`;
  if (nStates > 4)
    return `State count ${nStates} > 4 not supported; reduce in code or project yourself.`;
  if (nObs < 1 || nStates < 2)
    return "Need at least 1 observation and 2 states.";
